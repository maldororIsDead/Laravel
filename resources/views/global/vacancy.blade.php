                    ...
                    <div class="card-body">
                        <div class="container">
                            <div class="row justify-content-between mt-1 mb-3">
                                <div class="col-md-3">
                                    <a class="btn btn-outline-secondary" href="{{ route('all-vacancies')}}"
                                       role="button">Назад</a>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row justify-content-around mt-3 mb-3">
                            @if (Auth::check() && Auth::user()->isWorker())
                                <div class="row justify-content-center">
                                    <form action="{{ route('send-resume') }}" method="POST">
                                        {{ csrf_field() }}
                                        <div class="row justify-content-center align-items-end">
                                            <div class="form-group col-md-6">
                                                <label for="exampleFormControlSelect1">Выберите резюме</label>
                                                <select class="form-control" id="exampleFormControlSelect1"
                                                        name="resume_id">
                                                    @foreach ($resumes as $resume)
                                                        <option value="{{ $resume->id }}">{{ $resume->post }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="form-group col-md-6">
