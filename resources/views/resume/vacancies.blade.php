                                ...
                                <div class="col-md-12">
                                    <div class="card flex-md-row mb-4 box-shadow h-md-250">
                                        <div class="card-body d-flex flex-column align-items-start">
                                            ...
                                            <h3 class="mb-0">
                                                <a class="text-dark" href="{{ route('worker-vacancy', $vacancy->id) }}">{{ $vacancy->title }}</a>
                                            </h3>
                                            <div class="mb-1 text-muted">{{ $vacancy->company }}</div>
                                            <p class="card-text mb-auto">{{$vacancy->employment_type }}</p>
                                            @if (Auth::user()->isWorker())
                                                <a href="{{ route('worker-vacancy', $vacancy->id) }}">Просмотреть</a>
                                            @else
                                                <a href="{{ route('vacancy', $vacancy->id) }}">Просмотреть</a>
                                            @endif
                                        </div>
